const OG_IMAGE_PATTERN = /^https?:\/\/pix\.woolensheep\.top\/+og-image.*\.png$/i
...
const buildThumbnailUrl = (slug: string) => {
  const encodedOnce = normalizeSlug(slug)
  return `https://p.woolensheep.top/.afilmory/thumbnails/${encodedOnce}.jpg`
}
...
export const onRequest = async (context: { request: Request; next: () => Promise<Response> }) => {
  ...
  const thumbnailUrl = buildThumbnailUrl(slug)
